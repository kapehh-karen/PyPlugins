...
* Импортируемые кастомные модули кешируются, для продакшена это норм, но для тестирования это боль. Одно из решений перезагружать нужный модуль вручную во время разработки:
```python
# Перезагрузка модуля
import EntitiesRemover.remover
reload(EntitiesRemover.remover)

# А это уже наш нужный импорт, к примеру
from EntitiesRemover.remover import EntityRemover
